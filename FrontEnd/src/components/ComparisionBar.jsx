...
/**
 * Calculates a "nice" upper bound for a scale.
 * e.g., 87 -> 100, 4321 -> 5000, 950 -> 1000
 * @param {number} n The maximum value in the dataset.
 * @returns {number} A rounded-up, clean ceiling for the scale.
 */
...
const ComparisonBar = ({ label, value1, value2, user1Name, user2Name, isSmallerBetter = false }) => {
  ...
  // Determine the winner based on the metric type
  if (isSmallerBetter) {
    // For ranks, a value of 0 often means 'unranked'. A positive rank is always better than unranked.
    if (v1 > 0 && (v2 <= 0 || v1 < v2)) winner = 'user1';
    else if (v2 > 0 && (v1 <= 0 || v2 < v1)) winner = 'user2';
  } else { // Standard "bigger is better"
    if (v1 > v2) winner = 'user1';
    else if (v2 > v1) winner = 'user2';
  }

  // Use the new ceiling function to create a more readable scale
  const scaleMax = getScaleCeiling(Math.max(v1, v2, 1));

  let width1, width2;

  // Calculate bar widths. For "smaller is better", a lower value gets a longer bar.
  if (isSmallerBetter) {
    // Invert the value relative to the scale. A rank of 1 should be near 100%.
  ...
  // Define styles based on the winner
